	...
	while (!sharedPM->Shutdown.Flags.Laser and !kbhit()) {
        ...
        if (recvStr != "") {
            ...
            if (parseLaserData(recvStr, laserScanData)) {
                ...
                uint16_t resolution = std::stoi(laserScanData.at(RESOLUTION_INDEX), 0, 16);
                int32_t startAngle = std::stoi(laserScanData.at(STARTANGLE_INDEX), 0, 16);

                std::cout << "NumPoints " << numPoints << std::endl;
                std::cout << "Resolution " << resolution << std::endl;
                std::cout << "StartAngle " << startAngle << std::endl;

                for (int pointInd = 0; pointInd <= 4 /*numPoints*/; pointInd++) {
